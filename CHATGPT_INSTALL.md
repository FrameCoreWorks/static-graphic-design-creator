...
When approval for this installation has not already been given, before reading source files give this concise onboarding in the user's language and ask for clear conversational approval to create one Skill:
...
5. Verify the complete saved inventory and content, including the source identity, and retain the resolved manifest/source commits in host-supported installation evidence outside the canonical bundle. Report `installed` only when `@skill-creator` reports the Skill was created and saved, or when it is visible in the Skills library.
