...
Before writing, preserve the complete current Skill and the complete approved resulting content with its exact changed-path scope. Recheck current saved state before saving. After every save attempt, including an error or lost response, use supported bounded reconciliation and compare the actual saved inventory and bytes with both snapshots. An exact intended result is verified without another save; an unchanged baseline needs no remote rollback. Only a transient error with a freshly verified unchanged baseline permits considering one retry within existing approval and host limits, then stop after two attempts. Different or partial contents require a reviewed recovery that preserves later changes. Unavailable readback means verification is unavailable, with no blind retry or rollback. Do not use rebase for HTTP 500, force-push, change hooks or switch save services. These instructions govern recovery even if the installed version lacks a dedicated recovery reference or helper.

If saving remains blocked, report local preparation, actual observed persistence and recovery availability separately. Retain the approved content or reconstructable patch, baseline, intended/readback digests, target identity, approval and attempt history through an actually supported private recovery mechanism; verify any claimed durable checkpoint. Do not publish personal files or describe scratch as a durable backup. On resumption, first verify that the prepared files still exist and match approval, then compare the current saved state again without overwriting newer changes or resetting the retry limit. A local commit or a successful-looking message alone is not a saved extension.
```
...
Before writing, preserve the complete current Skill and the complete approved resulting content with its exact changed-path scope. Recheck current saved state before saving. After every save attempt, including an error or lost response, use this Codex host's actual save/readback mechanism and bounded reconciliation when applicable; do not assume a ChatGPT storage branch or network save. Compare the actual saved inventory and bytes with both snapshots. An exact intended result is verified without another save; an unchanged baseline needs no rollback of persisted files. Only a transient error with a freshly verified unchanged baseline permits considering one retry within existing approval and host limits, then stop after two attempts. Different or partial contents require a reviewed recovery that preserves later changes. Unavailable readback means verification is unavailable, with no blind retry or rollback. Do not use rebase for HTTP 500, force-push, change hooks or switch save services. These instructions govern recovery even if the installed version lacks a dedicated recovery reference or helper.
