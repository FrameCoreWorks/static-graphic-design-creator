...
## Optional multi-asset project

For explicitly separated production, [layered assets](layered-assets-workflow.md) owns the shared composition, ordered sequence, dependency graph, approval/version registry and delivery inventory. Its plan is separate from this single-asset schema. Use this canonical state for the current raster operation only; keep full project copy/locks and manual text assignments in the shared plan. Asset-level `no_copy` must not erase project text. Resolve only decisions affecting that asset before execution; assess the whole composition before calling it complete.

A pending asset render is still one native call. After its result, review and wait for explicit user acceptance or a requested correction before progressing to the next agreed asset. Preserve the approved selection through replacement candidates. Registry values never manufacture approval evidence. Planning, packaging and text specifications use the requested non-render scope; packaging is not a new schema render action. Recheck existing local extensions for a concrete conflict rather than silently running both workflows.

## Scoped-edit state

Catalog browsing, manual selection and optional internal selection from an ordinary brief use [the code workflow](event-poster-code-workflow.md). A code-list request uses `request_kind: advice`, `action: answer` and `output_mode: none`. Put only user-selected canonical codes in `strategy.style_requests`; retain assistant-selected entries, origin and visibility in internal lookup context and put their interpreted choices in existing strategy fields. Do not promote an inferred style into a user request or concept lock. Hidden-code preference applies to prose, prompt and technical handoff exposure; internal state is not a mandatory user-facing dump. Use `task_mode: variation` for a requested broad source-poster rebuild, `generate` for a new brief, and `edit` for a narrow change. Do not add a `rebuild` mode or schema keys. Preserve source and per-property/copy locks; automatic catalog selection grants no execution or redesign permission. Respect catalog opt-out.
