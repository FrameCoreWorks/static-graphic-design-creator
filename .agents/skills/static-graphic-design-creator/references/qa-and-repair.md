...
For observed repeated patterns, material collapse or smoothing, read [material artifact diagnosis](material-artifact-diagnosis.md). Inspect the original where possible and distinguish original defects from screenshot, compression or upscale effects. Assess unwanted artifact, intended detail and creative intent separately. Preserve deliberate halftone, gingham and pixel structure; a global smoothing pass cannot establish a successful repair.

When transparency is required, inspect the actual alpha channel and edges on light and dark backgrounds. A painted checkerboard or a filename ending in PNG is not proof of transparency. Check fine hair, glass, edge contamination and the intended handling of shadows/reflections where relevant; keep unmeasurable properties `Unknown`.
...
For serial work, also compare against the original approved base to catch cumulative changes in age, construction, type or texture. Do not accept a scarf-color change when the protected face or garment changes, or use a rejected edit as the next base. Higher model quality and a fresh conversation do not guarantee repair success.
